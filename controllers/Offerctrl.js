...
const Offerctrl = () => {
    ...
    try {
      ...
      for (var val of data) {
        const product = await products.findOne({
          $and: [{ product_name: val.product_name }],
        });
  ...
  const special_offers = async (req, res) => {
    try {
      ...
      APIResp.getSuccessResult(
        data,
        "special offers listed out successfully",
        res
      );
    } catch (error) {
  ...
  const offer_related_products = async (req, res) => {
    try {
      let userInput = helper.getReqValues(req);
      ...
      APIResp.getSuccessResult(
        data,
        "products and it's related offers listed successfully",
        res
      );
    } catch (err) {
      console.log(err);
